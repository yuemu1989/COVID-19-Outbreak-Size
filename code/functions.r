...
modelmean = function(pars,surv=surveillance)
{
  mu1 = pars$n0*pars$p1*exp(pars$b*(surv$t-pars$L1))
  mu2 = pars$n0*pars$p2*exp(pars$b*(surv$t-pars$L2))
  return(list(mu1=mu1,mu2=mu2))
}

loglikelihood = function(pars,surv=surveillance)
{
  mus = modelmean(pars,surv)
  out1 = out2 = 0
  i=which(!is.na(surv$s1))
  if(length(i)>0)out1 = sum(dpois(surv$s1[i],mus$mu1[i],log=TRUE))
  i=which(!is.na(surv$s2))
  if(length(i)>0)out2 = sum(dpois(surv$s2[i],mus$mu2[i],log=TRUE))
...
mcmc = function(pars,surveillance,MCMCits=10000,SUBits=100)
{
  ...
  for(iteration in 1:MCMCits)
  {
    ...
    storepars$size[iteration]=casestodate(pars,max(surveillance$t))
    storepars$sizetoday[iteration]=casestoday(pars,max(surveillance$t))
  }
